...
export const handleCallback = async (req: express.Request, res: express.Response) => {
  ...
  try {
    await verifySubcription(<string>req.query.ba_token, <string>req.query.subscription_id, <string>req.query.token);
    res.status(200).json({ subscriptionId: req.query.subscription_id, 'status': 'ACTIVE' });
  }
  catch (ex) {
    res.status(400);
  }
